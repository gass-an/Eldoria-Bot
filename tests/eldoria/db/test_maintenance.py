...
@pytest.fixture
def mod():
    m = importlib.import_module("eldoria.db.maintenance")
    importlib.reload(m)
    return m
...
def test_backup_to_file_raises_if_checkpoint_fails(monkeypatch, mod):
    ...
    main.execute = execute_maybe_fail
...
def test_replace_db_file_cross_device_fallback(monkeypatch, mod):
    ...
    monkeypatch.setattr(mod.os, "replace", fake_replace, raising=True)
    monkeypatch.setattr(mod.os, "makedirs", lambda p, exist_ok=False: calls["makedirs"].append((p, exist_ok)), raising=True)
